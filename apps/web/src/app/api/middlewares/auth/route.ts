...
export async function middleware(req: NextRequest) {
  ...
  try {
    ...
    headers.set("x-user-id", existingUser.id);
    if (existingUser.teacher?.id) headers.set("x-teacher-id", existingUser.teacher.id)
    if (existingUser.student?.id) headers.set("x-student-id", existingUser.student.id);
